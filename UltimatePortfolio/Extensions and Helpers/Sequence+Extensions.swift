...
extension Sequence {
    ...
    func sorted(by sortDescriptor: NSSortDescriptor, as comparisonResult: ComparisonResult = .orderedAscending) -> [Element] {
        self.sorted {
            sortDescriptor.compare($0, to: $1) == comparisonResult
        }
    }
    
    func sorted(by sortDescriptors: [NSSortDescriptor], ascending: Bool = true) -> [Element] {
        self.sorted {
